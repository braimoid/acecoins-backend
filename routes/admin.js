...
router.get("/manage/pause/:investmentId", middleware.isAdmin, (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "INvestment has been updated, will not mature" });
  // req.flash(
  //     'success',
...
router.get(
  "/manage/reactivate/:investmentId",
  middleware.isAdmin,
  (req, res) => {
    Investments.findById(req.params.investmentId, (err, investment) => {
    ...
    // res.redirect('back');
    res.status(200).json({
      message: "Investment has been updated, Investment will accumulate Profit",
    });
  }
);
...
router.get(
  "/admin/withdrawal/decline/:withdrawalId",
  middleware.isAdmin,
  (req, res) => {
...
router.get(
  "/admin/withdrawal/pay/:withdrawalId",
  middleware.isAdmin,
  (req, res) => {
    Withdrawal.findById(req.params.withdrawalId, (err, request) => {
      request.status = "Paid";
...
router.post("/admin/fund", (req, res) => {
  ...
  // res.redirect('/admin')
  res.status(200).json({ message: `Deposit of ${amount} was added to user` });
});
...
cron.schedule("*/30 * * * *", () => {
  ...
  Wallet.find({ isChecked: false }, (err, deposits) => {
    deposits.forEach((deposit) => {
      fetch(`https://api.nowpayments.io/v1/payment/${deposit.transactionId}`, {
        method: "get",
