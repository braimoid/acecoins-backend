...
router.post("/invest", middleware.isLoggedIn, (req, res) => {
  ...
  if (req.user.userStatus !== "Verified") {
    // req.flash(
    //   "error",
    //   "User is not Verified check your Email for our verification email or contact Support"
    // );
  ...
  if (req.body.pin == req.user.pin) {
      ...
      User.findById(req.user.id, (err, user) => {
        ...
        if (req.user.referral && user.referralPaid != true) {
          ...
          User.findOne({ referralId: req.user.referral }, (err, user, next) => {
            ...
            const earned = Math.min(invest.amount / 10, 500);
    ...
    // res.redirect("back");
    res.status(400).json({ error: true, message: "Incorrect Pin" });
  }
});
    ...
    .then((refs) => {
      Investments.find({ "user.id": req.user.id }).exec((err, investments) => {
        ...
        res.status(200).json({
          ...
          status: req.user.userStatus,
          balance: req.user.balance,
          profit: req.user.profit,
          refEarned: req.user.refEarned,
          refId: req.user.referralId,
          pin: req.user.pin,
          email: req.user.username,
  ...
  fetch("https://api.nowpayments.io/v1/payment", {
    ...
    .then((json) => {
      ...
      res.status(200).json({
        ...
        message: "Complete Deposit in the next page.",
      });
    });
});
...
router.post(
  ...
  function (req, res, next) {
    ...
    if (!validate(req.body.address)) {
      ...
      return res
        .status(400)
        .json({ error: true, message: "Enter a valid BTC address" });
    }
    if (Number(req.body.pin) !== req.user.pin) {
      // req.flash("error", "incorrect pin");
      return res.status(400).json({ error: true, message: "Incorrect Pin" });
    }
    if (req.user.balance < amount) {
      // req.flash("error", "You do not have sufficient balance");
      return res
        .status(400)
        .json({ error: true, message: "You do not have sufficient balance" });
    } else {
      ...
      res.status(200).json({
        success: true,
        message: "Withdraw successful. processing may take up to 24 hours",
      });
    }
  }
);
